...
Models:
    ${MVEI_MODEL_ROOT}/GLM-4.1V-9B-Thinking
    ${MVEI_MODEL_ROOT}/GLM-4.6V-Flash
...
* GLM-4V uses HF `AutoProcessor` + `apply_chat_template` for the
  <|begin_of_image|><|image|><|end_of_image|> placeholder expansion.

Usage (identical interface to internvl3_5.py):
    model = GLM4VInferencer(model_size="4_1v_thinking")
...
# Always 1 GPU per replica (pure DP). GLM-4V-9B fits ~18GB bf16, easy single card.
TP_PER_REPLICA: Dict[str, int] = {"4_1v_thinking": 1, "4_6v_flash": 1}

# Default precision per size. Override via env GLM4V_QUANT=bf16|int8|int4.
DEFAULT_QUANT: Dict[str, str] = {"4_1v_thinking": "bf16", "4_6v_flash": "bf16"}


# =============================================================================
# transformers.video_utils.VideoMetadata kwargs hot-patch
# (some checkpoints' processor passes unknown kwargs like frames_indices=)
# =============================================================================
...
def _worker_main(
    ...
    try:
        ...
        # patch unknown video kwargs (must be after transformers import)
        _patch_video_metadata_kwargs()

        # Architecture: GLM-4.1V uses Glm4vForConditionalGeneration
        try:
            from transformers import Glm4vForConditionalGeneration as _ModelCls
        except ImportError:
            # Fallback for newer/older naming
        ...
        def _build_messages(prompt: str, images: List["Image.Image"]) -> List[dict]:
            """GLM-4V chat-template content: list of {type: image|text}."""
        ...
        for sid, sample in shard.items():
            try:
                ...
                # Use processor's chat template (handles <|begin_of_image|>... insertion)
                ...
                for k, v in list(inputs.items()):
                    if isinstance(v, _torch.Tensor) and v.dtype in (_torch.float32, _torch.float16, _torch.float64):
                        inputs[k] = v.to(dtype)
                ...
                # Strip the prompt prefix from generated ids
                input_len = inputs["input_ids"].shape[1]
                gen_ids = out_ids[0][input_len:]
                out_text = tokenizer.decode(gen_ids, skip_special_tokens=True).strip()

            except Exception as e:
                out_text = f"[GLM4V-ERROR] {type(e).__name__}: {e}"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        m = GLM4VInferencer(model_size="4_1v_thinking")
